...
import type { Attendance, StoredWish } from "@/lib/wishes-store";
...
function getCountdown(): Countdown {
  const gap = Math.max(0, new Date(wedding.dateISO).getTime() - Date.now());
  return {
...
function EventSection() {
  return (
    <section className="event-section paper-section" id="event">
      ...
      <article className="unduh-card reveal">
  ...
  useEffect(() => {
    ...
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      if (event.key === "ArrowLeft" || event.key === "ArrowRight") {
        ...
        setActivePhotoIndex((current) => current === null ? 0 : (current + direction + wedding.galleryPhotos.length) % wedding.galleryPhotos.length);
      }
    };
    ...
    <>
      <section className="gallery-section paper-section" id="gallery">
        ...
        <div className="gallery-grid">
          {wedding.galleryPhotos.map((photo, index) => (
            <button
...
function WishesSection() {
  ...
  useEffect(() => {
    ...
    const loadWishes = async () => {
      try {
        const response = await fetch("/api/wishes", { cache: "no-store", signal: controller.signal });
  ...
  return (
    <section className="wishes-section paper-section" id="wishes">
      {celebrating && (
      ...
      <form className="wish-form reveal" onSubmit={submitWish}>
        ...
        {formMessage && <p className="wish-form-message" role="status">{formMessage}</p>}
      ...
      <div className="wish-list" aria-live="polite">
        {loadingWishes ? (
          <p className="empty-wishes reveal">Memuat ucapan…</p>
        ) : wishes.length === 0 ? (
          <p className="empty-wishes reveal">Jadilah yang pertama mengirimkan doa terbaik.</p>
